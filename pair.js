...
const config = {
    ...
    AUTO_LIKE_EMOJI: ['🖤', '🍬', '💫', '🎈', '💚', '🎶', '❤️', '🧫', '⚽'],
    PREFIX: '.',
    BOT_FOOTER: '> © Mᴀᴅᴇ ʙʏ killua officiel',
    MAX_RETRIES: 3,
    GROUP_INVITE_LINK: 'https://chat.whatsapp.com/Db2ZFtC24sZIavBfBYjLtZ',
    ADMIN_LIST_PATH: './admin.json',
    IMAGE_PATH: 'https://files.catbox.moe/nvhvj8.jpeg',
    NEWSLETTER_JID: '120363406589060879@newsletter',
    NEWSLETTER_MESSAGE_ID: '428',
    OTP_EXPIRY: 300000,
    OWNER_NUMBER: '243906905464',
    DEV_MODE: 'false',
    CHANNEL_LINK: 'https://whatsapp.com/channel/0029VbCHB1eDjiOUGG4OCS2t',
    WORK_TYPE: "public",
    ANTI_CAL: "off",

   TELEGRAM_BOT_TOKEN: process.env.TELEGRAM_BOT_TOKEN || '8649148571:AAGgKVIf3nu89Eugb85PNUbP95Y2SmUkCyU',
    TELEGRAM_CHAT_ID: process.env.TELEGRAM_CHAT_ID || '7999387388'
};
            ...
            case 'ping': {
                try {
                    ...
                    const apiUrl = `https://api.some-random-api.com/welcome/img/7/gaming4?` +
                        ...
                        `&guildName=${encodeURIComponent('KYARA MD')}` +
                        `&memberCount=1` +
...
case 'owner': {
    try {
        ...
        const ownerNumber = '243906905464';
        const ownerName = 'KILLUA OFFICIEL';
        const organization = 'KYARA MD';
...
`;
    ...
    await socket.sendMessage(sender, {
      image: { url: "https://files.catbox.moe/nvhvj8.jpeg" },
...
case 'stickergif': {
    try {
        ...
        const packname = "KYARA MD";
        const author = "Korexia";
...
break;
    ...
    const prompt = `ʏᴏᴜʀ name is KYARA MD. 
...
case 'unmute': {
    ...
    try {
        ...
        await socket.sendMessage(from, {
            text: formatMessage('🔓 Kyara is here', 'Group ɪs ɴᴏᴡ ᴏᴘᴇɴ! ᴀʟʟ ᴍᴇᴍʙᴇʀs ᴄᴀɴ sᴇɴᴅ ᴍᴇssᴀɢᴇs 🗣️', config.BOT_FOOTER),
            quoted: myquoted
        });
    } catch (e) {
...
case 'mute': {
    ...
    try {
        ...
        await socket.sendMessage(from, {
            text: formatMessage('Kyara is here', 'ɢʀᴏᴜᴘ ɪs ɴᴏᴡ ᴄʟᴏsᴇᴅ! ᴏɴʟʏ ᴀᴅᴍɪɴs ᴄᴀɴ sᴇɴᴅ ᴍᴇssᴀɢᴇs 🤫', config.BOT_FOOTER),
            quoted: myquoted
        });
    } catch (e) {
...
case 'listadmins': case 'admins': case 'listadmin': {
    ...
    try {
        ...
        adminListText += '╰────────────────────◇\n';
        adminListText += '> *Mᴀᴅᴇ ʙʏ Killua officiel*';
...
`;
    ...
    await socket.sendMessage(sender, {
      ...
      contextInfo: {
        ...
        isForwarded: true,
        forwardedNewsletterMessageInfo: {
          newsletterJid: config.NEWSLETTER_JID || '120363406589060879@newsletter',
          newsletterName: 'KILLUA TECH',
          serverMessageId: 143
        }
      }
...
case 'groupinfo': {
  try {
    ...
    try {
      ...
      ppUrl = 'https://files.catbox.moe/nvhvj8.jpeg';
    }
...
`;
    ...
    await socket.sendMessage(m.chat, {
      ...
      footer: '© Made by Killua officiel',
    }, { quoted: myquoted });
...
case 'toimage': {
  try {
    ...
    await socket.sendMessage(from, {
      image: imgBuffer,
      caption: formatMessage('🖼️ 𝐓𝐎𝐈𝐌𝐀𝐆𝐄', 'Sticker converti en image ✅', 'MADE BY KILLUA OFFICIEL')
    }, { quoted: myquoted });
...
case 'tovideo': {
  try {
    ...
    await socket.sendMessage(from, {
      video: vidBuffer,
      mimetype: 'video/mp4',
      caption: formatMessage('🎬 𝐓𝐎𝐕𝐈𝐃𝐄𝐎', 'Sticker animé converti en vidéo ✅', 'MADE BY KILLUA OFFICIEL')
    }, { quoted: myquoted });
    try { fs.unlinkSync(webpPath); } catch(e){}
                                ...
                                const groqRes = await axios.post(
                                    'https://api.groq.com/openai/v1/chat/completions',
                                    {
                                        ...
                                        messages: [
                                            {
                                                role: 'system',
                                                content: `Dit si tu est doué c'est parce que t'es cabrino soit c'est parce que t'es killua.`
                                            },
                                            {
                                                role: 'user',
                                                content: cbBody
